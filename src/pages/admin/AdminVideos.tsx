...
import { Link2, Trash2, Eye, EyeOff, Heart, X } from "lucide-react";
...
export default function AdminVideos() {
  ...
  return (
    <AdminLayout title="Gerenciar Vídeos">
      {/* Add via link */}
      <div className="bg-[#1A1A1A] rounded-2xl p-6 border border-[#222] mb-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
          <input value={linkTitle} onChange={e => setLinkTitle(e.target.value)} placeholder="Título do vídeo" className="bg-[#111] border border-[#333] rounded-lg px-3 py-2 text-sm text-white" />
          <input value={linkUrl} onChange={e => setLinkUrl(e.target.value)} placeholder="https://youtube.com/watch?v=..." className="bg-[#111] border border-[#333] rounded-lg px-3 py-2 text-sm text-white" />
        ...
        {linkUrl && getYoutubeThumbnail(linkUrl) && (
          <div className="mt-3 flex items-center gap-3">
            ...
            <span className="text-xs text-[#888]">Preview do vídeo</span>
          </div>
        )}
      </div>
      ...
      <div className="bg-[#1A1A1A] rounded-2xl border border-[#222] overflow-hidden">
        <table className="w-full text-sm">
          <thead><tr className="border-b border-[#222]">
            <th className="text-left p-4 text-[#666]">Vídeo</th>
            <th className="text-left p-4 text-[#666]">Tipo</th>
            <th className="text-center p-4 text-[#666]">Curtidas</th>
            <th className="text-left p-4 text-[#666]">Data</th>
            <th className="text-right p-4 text-[#666]">Ações</th>
          </tr></thead>
          <tbody>
            {loading ? (
              <tr><td colSpan={5} className="p-8 text-center text-[#666]">Carregando...</td></tr>
            ) : videos.map(v => (
              <tr key={v.id} className="border-b border-[#222] last:border-0">
                ...
                <td className="p-4 text-[#888]">{v.video_type}</td>
                <td className="p-4 text-center text-primary font-bold">{v.likes_count}</td>
                <td className="p-4 text-[#888]">{new Date(v.published_at).toLocaleDateString('pt-BR')}</td>
                <td className="p-4 text-right">
                  <div className="flex gap-1 justify-end">
                    ...
                    <button onClick={() => setZerVideo(v)} className="p-2 rounded hover:bg-white/5"><Heart className="w-4 h-4 text-[#666]" /></button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80" onClick={() => setDeleteVideo(null)}>
          <div className="bg-[#1A1A1A] rounded-2xl p-6 max-w-sm w-full mx-4 border border-[#333] text-center" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-3 justify-center">
              <button onClick={() => setDeleteVideo(null)} className="px-4 py-2 text-sm text-[#888]">Cancelar</button>
      ...
      {zerVideo && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80" onClick={() => setZerVideo(null)}>
          <div className="bg-[#1A1A1A] rounded-2xl p-6 max-w-sm w-full mx-4 border border-[#333] text-center" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-3 justify-center">
              <button onClick={() => setZerVideo(null)} className="px-4 py-2 text-sm text-[#888]">Cancelar</button>
