...
export function MediaUploader({
  ...
  const upload = useCallback(
    async (file: File) => {
      ...
      try {
        // Tenta via XHR para barra de progresso real
        ...
        await new Promise<void>((resolve, reject) => {
          ...
          xhr.onload = () => {
            if (xhr.status >= 200 && xhr.status < 300) {
              ...
              // Falha no XHR — tenta com SDK do Supabase como fallback
        ...
        // ── Fallback: SDK do Supabase (sem barra de progresso) ──
        try {
          setProgress(50); // indicador visual durante fallback
          const { error } = await supabase.storage
            .from(bucket)
            .upload(path, file, { upsert: true, contentType: file.type || undefined });

          if (error) {
            // Mensagem amigável para erro de política RLS
            const msg =
              error.message?.includes("policy") ||
              error.message?.includes("Unauthorized") ||
              error.message?.includes("403")
                ? "❌ Erro de permissão no Storage.\nAplique a migration SQL de correção no Supabase Dashboard (arquivo: 20260530120000_fix_storage_rls_upload.sql)."
                : error.message;
            throw new Error(msg);
          }
          ...
          const raw: string = sdkErr?.message ?? String(sdkErr);
          let friendlyMsg = raw;
          if (
            raw.includes("policy") ||
            raw.includes("Unauthorized") ||
            raw.includes("403") ||
            raw.includes("row violates")
          ) {
            friendlyMsg =
              "Erro de permissão (RLS). Execute a migration SQL de correção no Supabase Dashboard para liberar os uploads.";
          }
          toast({
  ...
  const isVideo =
    preview && /\.(mp4|webm|mov|avi|mkv)(\?|$)/i.test(preview);

  return (
    <div className="w-full">
      {label && !compact && (
        <label className="block text-xs text-[#A1A1AA] mb-2">{label}</label>
      )}
      <div
        onDragOver={(e) => {
          e.preventDefault();
        ...
        {preview && !uploading ? (
          ...
          <div className="text-center text-[#A1A1AA] py-4">
            ...
            <p className="text-xs mt-1">
              Sem limite de tamanho · qualquer resolução
            </p>
          </div>
        )}
      </div>
